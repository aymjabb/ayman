module.exports.config = {
  ...
  hasPermssion: 1,
  credits: "عمر",
  description: "اضافة ردود مرحة وساخرة",
...
module.exports.languages = {
  "en": {
    "misingKeyword": `🚨 أوه لا! لا تترك الفراغ يا بطل 😹 ${getRandomEmoji()}`,
    "shortcutExist": `😺 هذا الرد موجود أصلاً! جرب شيئًا جديدًا ${getRandomEmoji()}`,
    "requestResponse": `🙀 رد على هذه الرسالة بالكلمة التي تريد إضافتها كاختصار 🐱✨`,
    "addSuccess": `🎉 يا سلام! تم إضافة الرد الجديد السحري ${getRandomEmoji()}\n- إيدي: %1\n- الاختصار: %2\n- الجواب: %3`,
    "listShortcutNull": `🙀 القائمة فارغة! لا توجد اختصارات 😹`,
    "removeSuccess": `🗑️ تم حذف الرد بنجاح! ${getRandomEmoji(3)}`,
    "returnListShortcut": `📜 قائمة اختصارات المجموعة:\n [الاختصار] => [الجواب]\n\n%1`,
    "requestKeyword": `✨ رد على هذه الرسالة بالكلمة التي تريد إضافة رد لها 😺`
  }
}
...
module.exports.handleEvent = async function ({ event, api }) {
  ...
  const found = data.find(item => item.input.toLowerCase() == body.toLowerCase());
  if (found) {
    const message = [
      `😺 هاها! هذا الرد جاء تلقائياً 🐱: ${found.output}`,
      `🙀 يا للروعة! تلقائياً الرد: ${found.output}`,
      `✨ تلقائياً: ${found.output} ${getRandomEmoji()}`,
      `😹 سيرا تشان تقول لك: ${found.output} ${getRandomEmoji()}`
    ];
    return api.sendMessage(message[Math.floor(Math.random() * message.length)], threadID, messageID);
  }
}
...
module.exports.handleReply = async function ({ event, api, handleReply, getText }) {
  ...
  switch (handleReply.type) {
    case "requireInput": {
      ...
      const data = global.moduleData.shortcut.get(threadID) || [];
      if (data.some(item => item.input.toLowerCase() == body.toLowerCase())) return api.sendMessage(getText("shortcutExist"), threadID, messageID);
...
module.exports.run = function ({ event, api, args, getText }) {
  ...
  switch (args[0]) {
    ...
    case "-d": {
      ...
      else indexNeedRemove = dataThread.shortcuts.findIndex(item => item.input.toLowerCase() == (args.slice(1, args.length)).join(" ").toLowerCase() || item.id == (args.slice(1, args.length)).join(" "));
    ...
    case "-a": {
      ...
      const array = [];
      let n = 1;
      for (const single of data) array.push(`${n++}/ ${single.input} => ${single.output} ${getRandomEmoji()}`);
